...
describe('DuelRequest', () => {
    ...
    describe('Content', () => {
        ...
        test('should build a description for the embed', () => {
            ...
            expect(duelRequest.content.embeds![0].description).toBe('duel.challenge\nduel.action');
        });

        test('should set embed color', () => {
            expect(duelRequest.content.embeds).toHaveLength(1);
            expect(duelRequest.content.embeds![0].color).toBe(duelRequest['embedColor']);
        });
    });
        ...
        describe('Using buttons', () => {
            ...
            `(
                ...
                async ({ userId, valid }) => {
                    ...
                    expect(options!.filter!({ user: { id: userId } } as any)).toBe(valid);
                }
            );
